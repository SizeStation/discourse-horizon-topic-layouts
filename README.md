...
The image modifier owns masonry measurement and cleanup. It recalculates on image load, layout changes, and grid resizing, and removes its observer and span overrides when the image or layout changes. Gallery and Masonry place new/unread badges before the title, aligned with its first line. Titles wrap within the remaining width and are clamped to two lines; badges remain visible. Titles without badges keep the full width. They share their base grid styles; the containing table uses fixed layout so its intrinsic sizing cannot widen that grid. Layouts that flatten Horizon's footer also disable its mobile scroll-fade pseudo-element, which otherwise overlays cards and timestamps.
