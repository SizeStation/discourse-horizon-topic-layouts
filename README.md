...
The image modifier owns masonry measurement and cleanup. It recalculates on image load, layout changes, and grid resizing, and removes its observer and span overrides when the image or layout changes. Gallery and Masonry render the standard new/unread badges inline before the title through the `topic-link` wrapper outlet and hide the original trailing badges. The heading is clamped to two lines, and wrapped text returns to its full left edge rather than reserving a badge column. A word joiner keeps the badge with the first title character, including for long unbroken words. They share their base grid styles; the containing table uses fixed layout so its intrinsic sizing cannot widen that grid. Layouts that flatten Horizon's footer also disable its mobile scroll-fade pseudo-element, which otherwise overlays cards and timestamps.
...
Responsive tests set the CSS viewport through Playwright's `set_viewport_size`, not Capybara's outer-window `resize_to`. Browser minimum window sizes can otherwise produce a narrower viewport than requested. Assert `window.innerWidth` after navigation, when the mobile viewport meta tag has loaded, and before checking layout geometry.
